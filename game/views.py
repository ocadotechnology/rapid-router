...
def levels(request):
    context = RequestContext(request, {
        'levels': Level.objects.filter(anders=True).order_by('id'),
    })
...
def level(request, level):
    ...
    attempt = None
    lesson = None
    levelCount = Level.objects.filter(anders=True).count()
...
def level_new(request):
    ...
    if 'path' in request.POST:
        ...
        passedLevel = None
        if not request.user.is_anonymous() and hasattr(request.user.userprofile, 'student'):
            passedLevel = Level(name=10, path=path, owner=request.user.userprofile, anders=False)
        else:
            passedLevel = Level(name=10, path=path, anders=False)
        passedLevel.save()
