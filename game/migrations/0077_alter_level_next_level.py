...
class Migration(migrations.Migration):
    def add_next_level_to_last_levels(apps, schema_editor):
        ...
        levels = (
            Level_Object_Wrapper.filter(next_level=None)
            .exclude(name="109")
            .exclude(episode=None)
        )
        for level in levels:
            next_level_update = Level_Object_Wrapper.filter(
                name=(str(int(level.name) + 1))[0]
            )
    ...
    def remove_next_level_to_last_levels(apps, schema_editor):
        ...
        ).exclude(episode=None).update(next_level=None)
