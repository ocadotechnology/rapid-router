...
def change_levels(apps, schema_editor):
    ...
    level74.path = ('[{"coordinate":[9,2],"connectedNodes":[1]},' +
    ...
    set_decor(level74, json.loads('[{"x":738,"y":33,"decorName":"tree1"},' +
    ...
    set_blocks(level74, json.loads('[{"type":"move_forwards","number":1},' +
    ...
    levelBlock = LevelBlock.objects.get(type=Block.objects.get(type='controls_repeat_until'),
                                        level=level89)
    levelBlock.delete()

    level90 = Level.objects.get(name='90', default=1)
    level90.path = ('[{"coordinate":[1,6],"connectedNodes":[1]},' +
    ...
    Level.objects.filter(name='106', default=1).exclude(pk=level105.next_level.id).delete()

    level106 = Level.objects.get(name='106', default=1)
    Level.objects.filter(name='107', default=1).exclude(pk=level106.next_level.id).delete()
