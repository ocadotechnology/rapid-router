...
def _prev_level_url(level, user, night_mode):
    """
    Find the previous available level. Check if level is available, if so decrement the level
    """
...
<<<<<<< HEAD
        ...
        try:
            while (
                klass in prev_level.locked_for_class.all() and int(prev_level.name) > 1
            ):
