...
def dragAndDrop():
    return "Drag the blocks from the left to the white space and connect them with one another "
        ...
        + "To remove an instruction place it back in the gray area.\n Remember to start from the "
        + "'start' block."
...
def description_level5():
    message = "Now you're ready for more complex paths.\n"
    return message + description_overall

def description_level6():
    message = "Manualy adding repeating instructions is boring. That's why there is a 'repeat' "
        + "block.\n Repeat block executes the instructions attached inside it specified amount of "
        + "times. Type into the light green box in the block a number of repetitions.\n"
    return message + description_overall()

def description_level7():
    message = "This road looks quite familiar, doesn't it? Use the 'repeat' block to simplify your "
        + "program which guides the driver."
    return message + description_overall()
...
def description_level14():
    message = "Quite often we need to check more than one condition to know how to behave - "
        + "there might not be a turn right, but we still don't know whether we can go forwards or "
...
def description_level15():
    message = "Now you're ready! Use all your knowledge and the newly added conditions to guide "
